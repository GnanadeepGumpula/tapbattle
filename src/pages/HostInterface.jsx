...
const HostInterface = () => {
  ...
  try {
    ...
    const [tapsResponse, teamsResponse, playersResponse] = await Promise.all([
      ...
      api.getPlayersInSession(sessionId),   // ✅ correct
    ]);
    ...
    // ✅ solo players = no team OR joinMode single
...
const handleRemovePlayer = async (playerId) => {
  ...
  if (!confirmDelete) return; // ❌ User canceled, do nothing
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-row items-center justify-between mb-6 sm:mb-8 space-x-2 sm:space-x-3">
  <div className="flex items-center space-x-2 sm:space-x-3">
    <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800">Host Interface</h1>
    <p className="text-gray-600 text-xs sm:text-sm">Session: {sessionId}</p>
  </div>

  <div className="flex items-center space-x-2 sm:space-x-3">
    <button
      onClick={handleExportSession}
      className={`btn-secondary text-xs sm:text-sm px-2 py-1 sm:px-3 sm:py-1.5 flex items-center rounded-md transition-colors ${
      ...
      Back
        ...
        <div className="card mb-6 sm:mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-xl font-bold text-gray-800">Session Information</h2>
            <div className="flex items-center space-x-3 sm:space-x-4">
              <button
                onClick={() => setShowPlayerModeChange(true)}
                className="flex items-center px-2 sm:px-3 py-1 sm:py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors text-sm sm:text-base"
                disabled={isUpdatingRef.current}
              >
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs sm:text-sm text-gray-600">Round {session.round}</span>
              </div>
            </div>
          </div>
          <div className="grid md:grid-cols-4 gap-4">
            <div className="bg-blue-50 rounded-lg p-3 sm:p-4">
              <h3 className="font-semibold text-blue-800 mb-2 text-sm sm:text-base">Joining Code</h3>
              <div className="flex items-center space-x-2">
                <span className="text-lg sm:text-2xl font-bold text-blue-600">{sessionId}</span>
                <button
            ...
            <div className="bg-teal-50 rounded-lg p-3 sm:p-4">
              <h3 className="font-semibold text-teal-800 mb-2 text-sm sm:text-base">Total Players</h3>
              <div className="flex items-center space-x-2">
                <Users className="w-4 h-4 sm:w-5 sm:h-5 text-teal-600" />
                <span className="text-lg sm:text-xl font-bold text-teal-600">{players.length}</span>
              </div>
            </div>
            <div className="bg-purple-50 rounded-lg p-3 sm:p-4">
              <h3 className="font-semibold text-purple-800 mb-2 text-sm sm:text-base">Teams</h3>
              <div className="flex items-center space-x-2">
                <Shield className="w-4 h-4 sm:w-5 sm:h-5 text-purple-600" />
                <span className="text-lg sm:text-xl font-bold text-purple-600">{teams && teams.length}</span>
              </div>
            </div>
            <div className="bg-orange-50 rounded-lg p-3 sm:p-4">
              <h3 className="font-semibold text-orange-800 mb-2 text-sm sm:text-base">Player Mode</h3>
          ...
          <div className="mt-4 sm:mt-6 bg-gray-50 rounded-lg p-3 sm:p-4">
            <h3 className="font-semibold text-gray-800 mb-2 sm:mb-3 text-sm sm:text-base">Share Session</h3>
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <h4 className="text-xs sm:text-sm font-medium text-gray-600 mb-2">Join Link</h4>
                <div className="flex items-center space-x-2">
                  <span className="text-xs sm:text-sm text-blue-600 truncate">{joinLink}</span>
                  <button
              ...
              <div>
                <h4 className="text-xs sm:text-sm font-medium text-gray-600 mb-2">QR Code</h4>
                <div className="flex items-center space-x-2">
                  <div ref={qrCodeRef}>
            ...
            <div className="text-xs sm:text-sm text-gray-500 mt-2">Last updated: {lastUpdated}</div>
          </div>
          ...
          {playerMode === "teams" && (
            <div className="mt-4 p-3 sm:p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center text-yellow-800">
                ...
                <span className="font-semibold text-sm sm:text-base">Teams Only Mode Active</span>
              </div>
        ...
        {activeTab === "taporder" && (
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg sm:text-xl font-bold text-gray-800">Button Tap Order - Round {session.round}</h2>
              <button
                onClick={refreshRound}
                className={`flex items-center space-x-2 btn-accent text-sm sm:text-base ${
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {tapOrder.length > 0 ? (
                tapOrder.map((tap, index) => (
                  <div
                    key={tap.tapId}
                    className="flex items-center justify-between p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border-l-4 border-blue-500"
                  >
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <div>
                        <span className="font-semibold text-gray-800 text-sm sm:text-base">{tap.playerName}</span>
                        {tap.teamName && <div className="text-xs sm:text-sm text-teal-600">Team: {tap.teamName}</div>}
                        <div className="text-xs sm:text-sm text-gray-600">{tap.time}</div>
                      </div>
                    </div>
                ...
                <div className="text-center py-8 text-gray-500">
                  <Users className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 text-gray-400" />
                  <p className="text-sm sm:text-base">Waiting for participants to tap the button...</p>
        ...
        {activeTab === "teams" && (
          <div className="space-y-6">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg sm:text-xl font-semibold text-gray-800">Team Management</h3>
                <button
              ...
              {showTeamCreator && (
                <div className="bg-gray-50 p-3 sm:p-4 rounded-lg mb-4">
                  <h4 className="font-semibold text-gray-800 mb-3 text-sm sm:text-base">Create New Team</h4>
                  <div className="grid md:grid-cols-2 gap-4">
                    <input
                      type="text"
                  ...
                  <div className="flex space-x-4 mt-4">
                    <button
                      onClick={createTeam}
                      className={`btn-primary text-sm sm:text-base ${isDeleting ? "opacity-50 cursor-not-allowed" : ""}`}
            ...
            <div className="grid lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="lg:col-span-1">
                <div className="card">
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Teams ({teams && teams.length})</h3>
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div>
                                <h4 className="font-semibold text-gray-800 text-sm sm:text-base">{team.teamName}</h4>
                    ...
                    {teams && teams.length === 0 && (
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-xs sm:text-sm mt-2">Create teams for players to join</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {selectedTeam ? (
                  <div className="card">
                    <div className="flex items-center justify-between mb-4 sm:mb-6">
                      <div>
                        <h3 className="text-lg sm:text-xl font-bold text-gray-800">{selectedTeam.teamName}</h3>
                      ...
                      <div className="text-right">
                        <div className="text-xl sm:text-2xl font-bold text-blue-600">{teamPlayersList.length}</div>
                        <div className="text-xs sm:text-sm text-gray-600">Members</div>
                      </div>
                    </div>

                    <div>
                      <h4 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Team Members</h4>
                      {teamPlayersList.length > 0 ? (
                          ...
                          {teamPlayersList.map((player, index) => (
                            ...
                            >
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  <div className="font-semibold text-gray-800 text-sm sm:text-base">{player.playerName}</div>
                                  <div className="text-xs sm:text-sm text-gray-600">
                                    Joined: {new Date(player.createdAt).toLocaleDateString()}
                                  </div>
                                  {index === 0 && (
                                    <div className="text-xs sm:text-sm text-blue-600 font-medium">Team Leader</div>
                                  )}
                                </div>
                              </div>
                              <button
                        ...
                        <div className="text-center py-8 text-gray-500">
                          <Users className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 text-gray-400" />
                          <p className="text-sm sm:text-base">No members in this team yet</p>
                          <p className="text-xs sm:text-sm mt-2">Share team name and password with players</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="card text-center py-10 sm:py-12">
                    ...
                    <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-2">Select a Team</h3>
                    <p className="text-gray-600 text-sm sm:text-base">Choose a team from the list to view and manage its members</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {activeTab === "players" && (
          <div className="card">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-800 mb-4">Solo Players ({soloPlayers.length})</h3>
            {filteredSoloPlayers.length > 0 ? (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredSoloPlayers.map((player) => (
                  <div key={player.playerName} className="p-3 sm:p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-100 rounded-full flex items-center justify-center">
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm sm:text-base">No solo players yet</p>
                <p className="text-xs sm:text-sm">Players will appear here when they join as individuals</p>
              </div>
            )}
          </div>
        )}
        ...
        {showPlayerModeChange && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="card max-w-md w-full">
              <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-4">Change Player Mode</h3>
              ...
              <div className="space-y-3 mb-4 sm:mb-6">
                <button
                  ...
                  disabled={isDeleting || isUpdatingRef.current}
                >
                  <User className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-2 text-blue-600" />
                ...
                <button
                  ...
                  disabled={isDeleting || isUpdatingRef.current}
                >
                  <Users className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-2 text-teal-600" />
                ...
                <button
                  ...
                  disabled={isDeleting || isUpdatingRef.current}
                >
                  <UserPlus className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-2 text-purple-600" />
              ...
              <button
                ...
                className="w-full py-2 sm:py-3 px-4 sm:px-6 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-sm sm:text-base"
                disabled={isDeleting || isUpdatingRef.current}
              >
                Cancel
              </button>
